...
unsigned int extract_message_size(
  j_common_ptr info,
  jvirt_barray_ptr* coef_arrays
)
{
  // Potential message bits.
  ...
  return ( mbits / 8 ) + ( mbits % 8 );
}

const char* extract_message_f4(
  j_common_ptr info,
  jvirt_barray_ptr* coef_arrays
)
{
  unsigned int mbits = 0;
  unsigned char mbit;
  unsigned int message_size = extract_message_size( info, coef_arrays );
  unsigned char* message = calloc( message_size + 1, sizeof( char ) );
  message[ message_size ] = '\0';
  printf( "message_size is %u\n", message_size );
            ...
            for ( block = 0; block < blocks; ++block ) 
            {
               ...
               for ( coeff = 0; coeff < ( coeffs - 1 ); ++coeff )
               {
                  ...
                  if ( value != 0 )
                  {
                     ...
                     if ( mbits < 24 )
                     {
                        printf( "value: %08d, ", value );
                        printf( "extracted: 0x%02x\n", mbit );
                     }

                     mbit = ( mbit << ( 7 - ( mbits % 8 ) ) );
                     message[ mbits / 8 ] |= mbit;
                     ++mbits;
                  }
                  else
                  {
...
void embed_message_f4(
  j_common_ptr info,
  jvirt_barray_ptr* coef_arrays,
  const char* message
)
{
  unsigned int mbits = ( strlen( message ) + 1 ) * 8;
  unsigned int mbit = 0;
...
////////////////////////////////////////////////////////////////////////////////
  ...
  //printf( "embed message\n" );
  //embed_message_f4( (j_common_ptr)&srcinfo, src_coef_arrays, "Hello, world!" );
  printf( "extract message\n" );
  const char* message = extract_message_f4( (j_common_ptr)&srcinfo, src_coef_arrays );
  printf( "message: %s\n", message );
